...
class Configurator(metaclass=SingletonMeta):
    def __init__(
        ...
        self.log_path, self.logger = self._setup_logger(log_path or 'default.log')

        if args is None: args = self._parse_args()
        self.args = args


        self.output_dir = self._setup_output_directory(output_dir or args.outputDir)

        self.config = self._load_configuration(config_path or 'config.ini')

    def _parse_args(self) -> argparse.Namespace:
        parser = ArgumentParser()
        return parser.parse()

    def _setup_logger(self, log_filename: PathLike[AnyStr]) -> (PathLike[AnyStr], logging.Logger):
        logger = LoggingConfigurator(path_validator=PathValidator())
        return (os.path.abspath(log_filename), logger.set_logger(silent=False))

    def _setup_output_directory(self, output_dir: PathLike[AnyStr]) -> PathLike[AnyStr]:
