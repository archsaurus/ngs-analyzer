...
class VariantCaller(LoggerMixin, IVariantCaller):
    ...
    def call_variant(sample: SampleDataContainer, executor: Union[CommandExecutor, callable]): pass
...
class UnifiedGenotyperVariantCaller(VariantCaller):
    def call_variant(
        ...
        self.logger.warning(f"GATK UnifiedGenotyper depricated since version 4.0.0. Configure older versions for using it")
        """ for 3.x.x versions
            cmd=[self.configurator.config['java'], '-jar',
                 self.configurator.config['gatk'], 'UnifiedGenotyper',
                 ' -R'+configs[7],' -I '+outDir+'patient_'+patNum+'/patient_'+patNum+'.sorted.read_groups.realigned.recal.'+cutPrimers+'bam',
...
class FreebayesVariantCaller(VariantCaller):
    def call_variant(
        ...
        cmd=[
            ...
            '--ploidy', str(4),
            '--standard-filters',
            '--min-alternate-fraction', str(0.01), # util default value is 0.05
        ...
        if isinstance(executor, CommandExecutor): executor.run(alternate_cmd)
        elif isinstance(executor, type(callable)): executor(alternate_cmd)
