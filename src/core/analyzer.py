...
class Analyzer(metaclass=SingletonMeta):
    ...
    def prepareData(self, patient: PatientDataContainer) -> dict[str, dict[str, PathLike[AnyStr]]]:
        ...
        """
        trimmed_reads_pathes = self.trimAdapters(patient, configurator.args.outputDir)
        ...
        aligned_reads_path = self.alignReadsToReference(
            patient,
            configurator.config['reference'],
            os.path.abspath(os.path.join(configurator.args.outputDir,f"patient_{patient.id}"))
        )
    ...
    def trimAdapters(
        ...
        if not os.path.exists(patient.R1_source): msg = f"R1 reads file '{patient.R1_source}' not found. Abort"
            ...
            configurator.logger.info(f"Executing Trimmomatic command")
            configurator.logger.debug(f"Command: {trimmer_cmd}")

            self.cmd_caller(trimmer_cmd)

            configurator.logger.info(f"Trimming completed successfully. See the log at '{trimmer_summary_path}'")
    ...
    def alignReadsToReference(
        ...
        aligning_logpath = os.path.abspath(os.path.join(
            patient.processing_logpath,
            os.path.basename(os.path.splitext(configurator.config['bwa-mem2'])[0]))+'-mem'+'.log'
        )
        ...
        try:
            ...
            configurator.logger.debug(f"Command: {reads_mapping_cmd}")

            self.cmd_caller(reads_mapping_cmd)

            configurator.logger.info(f"Alignment completed successfully. See the log at '{aligning_logpath}'")

            return aligning_outpath
        except Exception as e:
            configurator.logger.critical(f"A fatal error '{e.__repr__()}' occured at '{e.__traceback__.tb_frame}'")
            exit(os.EX_SOFTWARE)
    ...
    def groupReads(
        ...
        picard_grouping_logpath = os.path.abspath(os.path.join(
            patient.processing_logpath,
            f"{os.path.splitext(os.path.basename(configurator.config['picard']))[0]}-AddOrReplaceReadGroups.log")
        )
        ...
        configurator.logger.info("Start grouping aligned reads")
        configurator.logger.debug(f"Command: {group_reads_cmd}")

        self.cmd_caller(group_reads_cmd)

        configurator.logger.info(f"Grouping reads has successfully done. See the log at '{picard_grouping_logpath}'")
    ...
    def performBQSR(
        ...
        base_recal_logpath = os.path.abspath(os.path.join(
            patient.processing_logpath,
            f"{os.path.basename(configurator.config['gatk'])}-BaseRecalibrator.log")
        )
        ...
        base_recal_cmd_str = ' '.join([
            configurator.config['gatk'], 'BaseRecalibrator',
            '--input', input_path,
            '--output', racalibration_table_path,
            '--reference', configurator.config['reference'],
            # TODO: Have to make it works with a list of sites
            '--known-sites', configurator.config['annotation-database'],
            '--intervals', configurator.config['chr13-interval'],
            '--intervals', configurator.config['chr17-interval'],
            '2>', base_recal_logpath
        ])

        try:
            configurator.logger.info("Executing BaseRecalibrator command")
            configurator.logger.debug(f"Command: {base_recal_cmd_str}")

            self.cmd_caller(base_recal_cmd_str)

            configurator.logger.info(f"BaseRecalibrator completed successfully. See the log at '{base_recal_logpath}'")
            ...
            apply_bqsr_cmd_str = ' '.join([
                base_recal_cmd_str.replace(
                    ...
                    configurator.config['annotation-database'], ''
                ),
                '--bqsr-recal-file', racalibration_table_path
            ])
            
            configurator.logger.info("Executing ApplyBQSR command")
            configurator.logger.debug(f"Command: {apply_bqsr_cmd_str}")
            
            self.cmd_caller(apply_bqsr_cmd_str)

            configurator.logger.info(f"ApplyBQSR completed successfully. See the log at '{apply_bqsr_logpath}'")

            return recalibrated_outpath
        except Exception as e:
            configurator.logger.critical(
