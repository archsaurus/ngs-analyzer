...
class LoggingConfigurator(ILoggingConfigurator):
    ...
    def set_logger(self, silent: bool=False) -> logging.Logger:
        try:
            if not self.args is None: base_logpath = os.path.abspath(os.path.join(self.log_path, self.args.logFilename))
            else: base_logpath = os.path.abspath(os.path.join(self.log_path, 'default_analyzer.log'))
            if not self.path_validator.verify_path(base_logpath, create_if_missing=True): exit(os.EX_SOFTWARE)
