...
class IPathValidator(ABC):
    @abstractmethod
    def verify_path(self, path: PathLike[AnyStr], create_if_missing: bool=False) -> bool: pass

class PathValidator(IPathValidator):
    def __init__(self, logger: logging.Logger=None):
        self.logger = logger if not logger is None else logging.getLogger(__name__)

    @staticmethod
    def verify_path(src: PathLike[AnyStr], create_if_missing: bool=False) -> bool:
        """
            ...
            Args:
                src (PathLike[AnyStr]): The path to the file or directory to check or create.
                create_if_missing (bool): Whether to create the path if it doesn't exist. Defaults to False.
            Returns:
        ...
        try:
            ...
            self.logger.critical(f"Error creating directory structure: '{e}'")
            return False
