...
from ngs_analyzer.core.base.helpers.archive_extraction import extract_archive
from ngs_analyzer.core.base.mixins.patterns import SingletonMeta
from ngs_analyzer.core.base.system.system_calls import touch
...
class DependencyHandler(metaclass=SingletonMeta):
    ...
    def set_logger(self, new_logger: logging.Logger) -> None:
        """Set a new logger for the handler, replacing the current one.
    ...
    def check_reference(
        ...
        def check_for_archive(
            ...
            """Check if the given file path points to an archive
            file (e.g., ZIP, TAR, GZ).
    ...
    def is_module_loaded(module_name: AnyStr) -> bool:
        """Check if a module is loaded in the current environment.
    ...
    def fetch_dependency(
        ...
        """Attempt to fetch a dependency module from pip, \
        ...
        match ans.lower():
            case 'y':
                ...
                logger.info(
                    'Module "%s" has successfully installed', module_name,
                )
    ...
    def resolve_file_path_by_extensions(
        ...
        """Search for the first existing file that matches \
