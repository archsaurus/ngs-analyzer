...
class SampleDataContainer:
    ...
    def __init__(
        ...
        """Initialize a sample data container.
    ...
    def parse_regions(
        ...
        """Parse target regions from a SAM file \
        ...
        """
        target_chromosomes = []
        default_sam_filepath = os.path.abspath(
            os.path.join(self.processing_path, self.sid + '.sam'),
        )
        try:
            with open(
            ...
            self.target_regions = tuple(filter(
                (lambda x: x),
                [
                    self.reg_tuple_generator(configurator, interval)
